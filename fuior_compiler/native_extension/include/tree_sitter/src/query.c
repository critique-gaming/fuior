...
static const TSQueryError PARENT_DONE = (TSQueryError)-1;
...
static inline StatePredecessorMap state_predecessor_map_new(const TSLanguage *language) {
  return (StatePredecessorMap) {
    .contents = (TSStateId*)ts_calloc(language->state_count * (MAX_STATE_PREDECESSOR_COUNT + 1), sizeof(TSStateId)),
  };
}
...
static inline void ts_query__pattern_map_insert(
  ...
  PatternEntry new_pattern_entry = ((PatternEntry) {
    .step_index = start_step_index,
    .pattern_index = pattern_index,
  });
  array_insert(&self->pattern_map, index, new_pattern_entry);
}
...
TSQuery *ts_query_new(
  ...
  if (ts_language_version(language) >= TREE_SITTER_LANGUAGE_VERSION_WITH_SYMBOL_DEDUPING) {
    ...
    symbol_map = (TSSymbol*)ts_malloc(sizeof(TSSymbol) * symbol_count);
  ...
  TSQuery *self = (TSQuery*)ts_malloc(sizeof(TSQuery));
  *self = (TSQuery) {
    .steps = array_new(),
...
TSQueryCursor *ts_query_cursor_new(void) {
  TSQueryCursor *self = (TSQueryCursor*)ts_malloc(sizeof(TSQueryCursor));
  *self = (TSQueryCursor) {
    .ascending = false,
    .halted = false,
    .states = array_new(),
...
static void ts_query_cursor__add_state(
  ...
  QueryState new_query_state = ((QueryState) {
  ...
  array_insert(&self->states, index, new_query_state);
}
