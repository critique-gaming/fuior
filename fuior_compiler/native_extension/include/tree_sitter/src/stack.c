...
static StackNode *stack_node_new(StackNode *previous_node, Subtree subtree,
  ...
  StackNode *node = (StackNode*)(pool->size > 0 ?
    array_pop(pool) :
...
Stack *ts_stack_new(SubtreePool *subtree_pool) {
  Stack *self = (Stack*)ts_calloc(1, sizeof(Stack));
...
inline StackAction iterate_callback(void *payload, const StackIterator *iterator) {
  StackIterateSession *session = (StackIterateSession*)payload;
  session->callback(
    session->payload,
    iterator->node->state,
...
inline StackAction pop_count_callback(void *payload, const StackIterator *iterator) {
  unsigned *goal_subtree_count = (unsigned*)payload;
...
inline StackAction summarize_stack_callback(void *payload, const StackIterator *iterator) {
  SummarizeStackSession *session = (SummarizeStackSession*)payload;
...
void ts_stack_record_summary(Stack *self, StackVersion version, unsigned max_depth) {
  SummarizeStackSession session = {
    .summary = (StackSummary*)ts_malloc(sizeof(StackSummary)),
    .max_depth = max_depth
  };
