...
void ts_external_scanner_state_init(ExternalScannerState *self, const char *data, unsigned length) {
  ...
  if (length > sizeof(self->short_data)) {
    self->long_data = (char*)ts_malloc(length);
...
ExternalScannerState ts_external_scanner_state_copy(const ExternalScannerState *self) {
  ...
  if (self->length > sizeof(self->short_data)) {
    result.long_data = (char*)ts_malloc(self->length);
...
void ts_subtree_array_copy(SubtreeArray self, SubtreeArray *dest) {
  ...
  if (self.capacity > 0) {
    dest->contents = (Subtree*)ts_calloc(self.capacity, sizeof(Subtree));
...
static SubtreeHeapData *ts_subtree_pool_allocate(SubtreePool *self) {
  if (self->free_trees.size > 0) {
    ...
    return (SubtreeHeapData*)ts_malloc(sizeof(SubtreeHeapData));
  }
}
...
MutableSubtree ts_subtree_clone(Subtree self) {
  ...
  Subtree *new_children = (Subtree*)ts_malloc(alloc_size);
  Subtree *old_children = (Subtree*)ts_subtree_children(self);
...
MutableSubtree ts_subtree_new_node(
  ...
  if (children->capacity * sizeof(Subtree) < new_byte_size) {
    children->contents = (Subtree*)ts_realloc(children->contents, new_byte_size);
...
char *ts_subtree_string(
  ...
  char *result = (char*)ts_malloc(size * sizeof(char));
