...
TSTree *ts_tree_new(
  ...
  TSTree *result = (TSTree*)ts_malloc(sizeof(TSTree));
  result->root = root;
  result->language = language;
  result->parent_cache = NULL;
  result->parent_cache_start = 0;
  result->parent_cache_size = 0;
  result->included_ranges = (TSRange*)ts_calloc(included_range_count, sizeof(TSRange));
...
void ts_tree_set_cached_parent(const TSTree *_self, const TSNode *node, const TSNode *parent) {
  ...
  if (!self->parent_cache) {
    self->parent_cache = (ParentCacheEntry*)ts_calloc(PARENT_CACHE_CAPACITY, sizeof(ParentCacheEntry));
  }
  ...
  self->parent_cache[index] = (ParentCacheEntry) {
    .child = (const Subtree*)node->id,
    .parent = (const Subtree *)parent->id,
    .position = {
      parent->context[0],
      {parent->context[1], parent->context[2]}
    },
    .alias_symbol = (TSSymbol)parent->context[3],
  };
